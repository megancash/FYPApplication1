...
public class AddTimetableActivity3 extends AppCompatActivity {
        ...
        reference.child(reference.push().getKey()).setValue(timetable).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful())
                    Toast.makeText(AddTimetableActivity3.this, "Slot added", Toast.LENGTH_SHORT).show();
                else
                    Toast.makeText(AddTimetableActivity3.this, "Could not add slot!", Toast.LENGTH_SHORT).show();
            }
        });
    }
    @Override
